...
Автоматическое создание проверяемого SQLite-снимка и скачивание ответов на
Windows-компьютер описаны отдельно в [`BACKUPS.md`](BACKUPS.md).
...
Затем перечитать карточки без ИНН или фамилии директора по сохраненной паре
...
частоты. Существующие `contractor_uuid` и данные директора не перезаписываются.
